...
export async function getComicsReading2(
  ...
  if ((!thumbnailUrl || thumbnailUrl === '') && parsed.listLink) {
    try {
      const parentUrl = parsed.listLink.startsWith('http')
        ? parsed.listLink
        : `${BASE_URL}${parsed.listLink}`;
      const parentResponse = await fetch(parentUrl, {
        headers: { 'User-Agent': deviceUserAgent },
        signal,
      });
      const parentData = await parentResponse.text();
      thumbnailUrl = await runOnRuntimeAsync(aniflixruntime, () => {
        'worklet';
        const $parent = cheerio.load(parentData, {
          xmlMode: true,
        });
        const thumb = $parent('.thumb img').attr('src') || '';
        return thumb ? (thumb.startsWith('http') ? thumb : `${BASE_URL}${thumb}`) : '';
      });
    } catch (error) {
  ...
  return {
    ...
    thumbnailUrl,
    comicImages: parsed.comicImages,
    nextChapter: parsed.nextLink
      ? parsed.nextLink.startsWith('http')
        ? parsed.nextLink
        : `${BASE_URL}${parsed.nextLink}`
      : undefined,
    prevChapter: parsed.prevLink
      ? parsed.prevLink.startsWith('http')
        ? parsed.prevLink
        : `${BASE_URL}${parsed.prevLink}`
      : undefined,
  };
}
