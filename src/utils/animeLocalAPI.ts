...
const fromUrl = async (url: string, selectedRes: RegExp | string = /480p|360p/, skipAutoRes = false, detailOnly = false, signal?: AbortSignal): Promise<AniStreaming | AniDetail | undefined> => {
    ...
    if (isAnimeDetail) {
        ...
        const m360p = mirrorStream.filter((i, el) => $(el).hasClass('m360p')).find('a').filter((i, el) => $(el).text().trim().startsWith('o')).attr('data-content');
        const m480p = mirrorStream.filter((i, el) => $(el).hasClass('m480p')).find('a').filter((i, el) => $(el).text().trim().startsWith('o')).attr('data-content');
        const m720p = mirrorStream.filter((i, el) => $(el).hasClass('m720p')).find('a').filter((i, el) => $(el).text().trim().startsWith('o')).attr('data-content');
