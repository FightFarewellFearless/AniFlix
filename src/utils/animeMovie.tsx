...
type Props = { isWebViewShown: boolean, setIsWebViewShown: (state: boolean) => void, onAnimeMovieReady: () => void };
let isError = false;
...
export async function getLatestMovie(signal?: AbortSignal, page?: number) {
  ...
  const response = await fetch(`https://154.26.137.28/movie-terbaru/${page ? `page/${page}/` : ''}`, {
    signal,
  });
...
export async function getMovieDetail(url: string, signal?: AbortSignal): Promise<MovieDetail> {
  ...
  const tableContents: { title: string, content: string }[] = []; table.each((i, el) => {
    tableContents[i] = {
...
export async function getStreamingDetail(url: string, signal?: AbortSignal) {
  ...
  mirror.each((i, el) => {
    const title = $(el).text().trim();
    if (title.toLowerCase().includes('pompom')) {
      pompomLinks.push({
        title,
        url: $(el).attr('data-em')!
      })
  ...
  mirror.each((i, el) => {
    const title = $(el).text().trim();
    if (title.toLowerCase().includes('pogo')) {
      pogoLinks.push({
        title,
        url: $(el).attr('data-em')!
      })
  ...
  return {
    ...
    streamingLink: isRawAvailable
      ...
      : cheerio
        .load(Buffer.from((supportedRawLinks.find(z => z.title.includes('480p')) ?? supportedRawLinks[0]).url, 'base64').toString('utf8'))
        ('iframe')
        .attr('src')!,
    resolution: (supportedRawLinks.find(z => z.title.includes('480p')) ?? supportedRawLinks[0]).title,
...
async function getAceRawData(acedata: string, signal?: AbortSignal) {
  ...
  const streamLink = JSON.parse(
    Buffer.from(
      resAceFileText
        .split('sources: JSON.parse(atob("')[1]
        .split('"')[0],
      'base64'
    ).toString('utf8')
  )[0].file;
...
async function getPixelOrPompomRawData(pixelorpompomdata: string, signal?: AbortSignal) {
  const url = cheerio.load(Buffer.from(pixelorpompomdata, 'base64').toString('utf8'))('iframe').attr('src')!;
  const response = await fetch(url, {
    signal,
  });
