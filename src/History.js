...
class History extends Component {
    ...
    componentDidMount() {
        this.unsubscribe = this.props.navigation.addListener('focus', this.updateHistory);
        this.updateHistory();
    }
            ...
            <View style={{ flex:1 }}>
                {
                    this.state.loading ? <ActivityIndicator /> :
                        ...
                        :
                            <FlatList data={this.state.data} keyExtractor={(item) => item.title} renderItem={({ item, index }) => {
                                return (
                                    <TouchableOpacity style={{ flexDirection: 'row', marginVertical: 5, backgroundColor: '#444242', borderRadius: 16 }} key={'btn' + item.title} onPress={(() => {
                                        this.props.navigation.dispatch(StackActions.push('FromUrl', {
                                            link: item.link,
                                        }));
                                    })}>
                                        <Image resizeMode='stretch' source={{ uri: item.thumbnailUrl }} style={{ width: 120, height: 210, borderTopLeftRadius: 16, borderBottomLeftRadius: 16, marginRight: 7 }} />
                                        ...
                                        <View style={{ flexDirection: 'column', flex: 1 }}>
                                            ...
                                            <View style={{ justifyContent: 'flex-end' }}>
                                                <Text style={{ color: '#1eb1a9', fontSize: 12 }}>{item.episode}</Text>
                                            </View>
                                            ...
                                            <View style={{ position:'absolute', right:5, top:5 }}>
                                                <TouchableOpacity hitSlop={10} onPress={() => {
                                                    Alert.alert('Yakin?', 'Yakin kamu ingin menghapus "' + item.title.trim() + '" dari histori?', [
                                                        {
                                                          text: 'Tidak',
                                                          onPress: () => null,
                                                          style: 'cancel',
                                                        },
                                                        {text: 'Ya', onPress: () => this.deleteHistori(index)},
                                                      ]);
                                                }}>
