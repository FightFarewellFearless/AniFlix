...
const defaultMessages = [
  {
    content:
      'SISTEM: Kamu adalah AniFlix Chat, sebuah aplikasi yang dibuat oleh FightFarewellFearless (Pirles).',
    role: 'user',
  },
  {
...
function TextCode(props: { text: string; textStyle?: TextStyle | undefined }) {
  ...
  return (
    <Text
      style={[
        { backgroundColor: colorScheme === 'dark' ? '#1b1b1b' : '#d1d1d1', padding: 5 },
        props.textStyle,
      ]}>
      {props.text}
    </Text>
...
function Chat(_props: Props) {
  ...
  const requestToGpt = useCallback(
    (
      prompt: string,
      callback: (text: string | undefined, isBingStreaming?: boolean, isDone?: boolean) => void,
    ) => {
      if (useBingAI) {
        requestBingAI(
          [...messagesHistroy.slice(-2), { content: prompt, role: 'user' }],
          data => {
            startTransition(() => {
              callback(data?.gpt, true, data.done);
            });
          },
          bingAbortRequest.signal,
        ).catch(err => {
    ...
    [bingAbortRequest.signal, messagesHistroy, useBingAI],
  );
  ...
  const tanya = useCallback(() => {
    ...
    setMessagesHistroy(old => [
      ...old,
      { content: prompt.current, role: 'user', source: 'Saya' },
      {
        content: 'Mohon tunggu sebentar...',
        role: 'assistant',
        source: useBingAI ? 'Bing' : 'AniFlix',
      },
    ]);
    delayScrollToEnd();
    requestToGpt(prompt.current, (text, isBingStreaming, isDone) => {
      setMessagesHistroy(old => {
        return [
          ...old.slice(0, -1),
          {
            content: text ?? 'TERJADI ERROR: Pastikan kamu terhubung ke internet dan coba lagi',
            role: 'assistant',
            source: useBingAI ? 'Bing' : 'AniFlix',
          },
        ];
    ...
    <View style={{ flex: 1, backgroundColor: colorScheme === 'dark' ? '#0A0A0A' : '#eeedf1' }}>
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.center}>
            ...
            <Text style={[globalStyles.text, { fontSize: 30, fontWeight: 'bold' }]}>
              Chat (Beta) {useBingAI ? '(Bing)' : ''}
            </Text>
            <Switch
              trackColor={{ false: '#767577', true: 'orange' }}
              thumbColor={colorScheme === 'dark' ? '#f5dd4b' : '#f4f3f4'}
              value={useBingAI}
              onValueChange={setUseBingAI}
            />
            <Text
              style={[
                globalStyles.text,
                ...
                !useBingAI ? { paddingBottom: 9 } : undefined,
              ]}>
              Gunakan Bing AI
            </Text>
            {useBingAI && (
              <Text
                style={[globalStyles.text, { fontSize: 15, paddingBottom: 9, color: 'orange' }]}>
                Kamu menggunakan fitur chat dari bing.
              </Text>
            )}
            <Text style={[globalStyles.text, { fontSize: 15, textAlign: 'center' }]}>
              AniFlix Chat adalah fitur chat yang ada di AniFlix bertujuan untuk memberikan
              pengalaman yang berbeda kepada pengguna.
            </Text>
            <Text style={[globalStyles.text, { fontSize: 15, textAlign: 'center' }]}>
              Note: AI tidak terbatas pada informasi seputar anime. Kamu bisa menanyakan apa saja ke
              AI.
            </Text>
          </View>
        }
        ...
        renderItem={({ item }) => {
          return (
            <Reanimated.View
              style={[
                item.role === 'user' ? styles.userResponse : styles.assistantResponse,
                { marginVertical: 5, maxWidth: '80%', minWidth: '40%', elevation: 3 },
              ]}
              entering={ZoomIn}>
              <Text
                style={{
                  color: colorScheme === 'dark' ? 'lightblue' : 'darkblue',
      ...
      <View style={{ flexDirection: 'row', maxHeight: '30%' }}>
        <TextInput
          ...
          style={[styles.textInput, { borderWidth: 1, borderColor: '#0099ff' }]}
          multiline
          onChangeText={t => {
            prompt.current = t;
          }}
        ...
        <TouchableOpacity
          style={styles.button}
          /* //rngh - containerStyle */ disabled={messageLoading}
          onPress={tanya}>
